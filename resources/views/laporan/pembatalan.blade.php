...
<div>
  ...
  <div class="card shadow mb-4">
    <div class="card-header py-3">
      <h6 class="m-0 font-weight-bold text-primary">Laporan Pembatalan</h6>
      <!-- <a href="{{ route('transaksis.create')}}" class="btn btn-primary ">
        <i class="fas fa-plus-square"></i> Tambah
      </a> -->
      <form class="form-inline" method="get" action="{{ route('laporan.pembatalan') }}">
    ...
    <div class="card-body" id="print-area">
      <div class="text-center my-5">
        <h3 class="text-primary font-weight-bold">Laporan Pembatalan</h3>
        ...
        <table class="table table-bordered" width="100%" cellspacing="0">
          ...
          <tbody>
            @foreach($pembatalans as $key => $pembatalan)
              <tr>
                <td>{{ $loop->index + 1 }}</td>
                <td>{{ $pembatalan->id_pembatalan }}</td>
                <td>{{ $pembatalan->transaksis->customers->nama ?? '' }}</td>
                <td>{{ $pembatalan->tanggal_batal }}</td>
                <td>{{ $pembatalan->transaksis->units->nama() }}</td>
                @if($lokasi == null)
                <td>{{ $pembatalan->transaksis->units->towers->lokasis->nama_apartemen }}</td>
                @endif
                <td>{{ $pembatalan->transaksis->jenis_bayar }}</td>
                <td>{{ $pembatalan->alasan }}</td>
                @if($pegawai == null)
                <td>{{ $pembatalan->transaksis->pegawais->nama ?? '' }}</td>
                @endif
                <td><span class="{{ $pembatalan->tgl_pengembalian != null ? 'text-success' : 'text-secondary' }}">{{ $pembatalan->tgl_pengembalian }}</span></td>
              </tr>
            @endforeach
      ...
      @if($tahun != null && $bulan != null)
      <p>Unit Dibatalkan: <span class="text-primary font-weight-bold">{{$pembatalans->count()}}</span></p>
      <p>Sudah Ditransfer: <span class="text-primary font-weight-bold">{{$pembatalans->where('gambar_bukti','!=',null)->count()}}</span></p>
      <p>Belum Ditransfer: <span class="text-primary font-weight-bold">{{$pembatalans->where('gambar_bukti',null)->count()}}</span></p>
      @endif
    </div>
  </div>
